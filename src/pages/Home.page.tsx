...
export function HomePage() {
  const { mode, user, initData, isAuthenticated, login, logout } = useAuth();
  ...
  const refreshData = () => {
    setLoading(true);
    getMain({ mode, initData, userId: user?.id })
      .then((res) => {
      ...
      <AppShell.Header>
        <Container py="sm">
          <Flex gap="md" direction="row" wrap="wrap">
            ...
            <User user={user ?? {}} balance={balance} mode={mode} onLogout={logout} />
          </Flex>
        </Container>
      </AppShell.Header>
      <AppShell.Main>
        ...
        <Products
          items={items}
          auth={{ mode, initData, userId: user?.id }}
          refreshData={refreshData}
          setNotification={(n: string | undefined) => setNotification(n)}
        />
      </AppShell.Main>
    </AppShell>
  );
